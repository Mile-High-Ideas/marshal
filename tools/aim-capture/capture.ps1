...
# Write to a guaranteed-local folder (never a Parallels shared/UNC path), and
# drop the final zip on the Desktop where Shane can find it easily.
$root = Join-Path $env:USERPROFILE "marshal-aim-capture"
$out  = Join-Path $root "aim-$stamp"
...
# ---------------------------------------------------------------------------
# 2. Every present USB / port / HID device, with status
# ---------------------------------------------------------------------------
...
Get-PnpDevice -PresentOnly |
  ...
  Format-Table -AutoSize | Out-File (Join-Path $out "usb-devices.txt") -Width 500
Get-PnpDevice -PresentOnly | Select-Object * | Export-Csv (Join-Path $out "pnp-all.csv") -NoTypeInformation
...
$aimFile = Join-Path $out "aim-device.txt"
...
if ($aim) {
  ...
  $note = @(
    ...
    "'Unknown Device' or a device with a Problem code, and note its InstanceId"
    "(that carries the USB VID/PID)."
  )
  $note | Out-File $aimFile -Append
}
...
# 4. Installed drivers
# ---------------------------------------------------------------------------
Section "Driver inventory"
pnputil /enum-drivers  | Out-File (Join-Path $out "pnputil-drivers.txt")
driverquery /v /fo csv | Out-File (Join-Path $out "driverquery.csv")

# ---------------------------------------------------------------------------
# 5. AiM .inf files - the driver model in plain text
# ---------------------------------------------------------------------------
Section "Locating AiM driver INF files"
$infList = Join-Path $out "aim-inf-matches.txt"
...
$rsFile = Join-Path $out "racestudio-install.txt"
...
# ---------------------------------------------------------------------------
# 7. Bundle onto the Desktop
# ---------------------------------------------------------------------------
Section "Bundling"
$desktop = [Environment]::GetFolderPath('Desktop')
$zip = Join-Path $desktop ("marshal-aim-capture-" + $stamp + ".zip")
